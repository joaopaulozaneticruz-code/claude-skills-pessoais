...
*(Nota de roteamento — 16/08/2026: "thought leader ads" aparece também no
gatilho da skill `influencer-marketing` com outro sentido — lá é sobre
encontrar/remunerar uma pessoa como thought leader B2B (o criador). Aqui em
`ads` é sobre o formato de anúncio pago (ex: LinkedIn Thought Leader Ads,
que promove um post orgânico de um funcionário/executivo como anúncio). Se
o pedido for "impulsionar um post existente como anúncio", é esta skill; se
for "encontrar/pagar um thought leader pra criar conteúdo", é
